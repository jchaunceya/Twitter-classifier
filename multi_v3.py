...
class class_object:
    def __init__(self, class_docs, all_vocab, class_num):
        ...
        self.num_docs = len(class_docs)
        self.class_tokens = []
        for document in class_docs:
            self.class_tokens += nltk.word_tokenize(document)
            self.doc_length.append(len(nltk.word_tokenize(document)))
        self.num_words = len(self.class_tokens)
        self.vocab = set(self.class_tokens)
        ...
        for word in self.vocab:
            self.freq_dist[word] = self.class_tokens.count(word)

        self.stddev = statistics.stdev(self.doc_length)
        self.mean = statistics.mean(self.doc_length)
...
def write_pred(obj_wrap):
    ...
    while buffer.qsize() < 10000:
        ...
        dateobject = dataObject(counters[1], counters[2], counters[0],counter, rtCounters[1], rtCounters[2], rtCounters[0])
